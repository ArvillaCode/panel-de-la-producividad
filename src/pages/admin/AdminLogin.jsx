...
const AdminLogin = () => {
  ...
  const { login, logout, getProfile, isAuthenticated, getUserInfo } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Verificar aprobación si no es admin
        if (formData.email !== 'admin@admin.com') {
          ...
          if (profile && !profile.is_approved) {
            await logout();
            throw new Error('Tu cuenta está registrada pero aún no ha sido aprobada por un administrador.');
          }
          if (profile && profile.status === 'inactive') {
            await logout();
            throw new Error('Tu cuenta está desactivada temporalmente.');
          }
        }

        if (rememberMe) {
          localStorage.setItem('loginCredentials', JSON.stringify({ email: formData.email, rememberMe: true }));
        } else {
        ...
        // Pequeña espera para asegurar que el estado de Auth se actualice
        setTimeout(() => {
          const targetPath = formData.email === 'admin@admin.com' ? '/admin/dashboard' : '/';
      ...
      setLoading(false); // Asegurar que se detiene el spinner si hay error
    }
    // No usamos finally aquí para que el spinner se quede si es exitoso (mientras redirige)
  };
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-700">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400">Regístrate para acceder</p>
          </div>
          <RegisterForm onSuccess={() => setIsRegistering(false)} onCancel={() => setIsRegistering(false)} />
          <button onClick={() => setIsRegistering(false)} className="w-full mt-4 text-blue-400 text-sm hover:underline">
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Panel de Productividad</p>
        </div>
        ...
        <div className="bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-700">
          ...
          <div className="mt-6 pt-6 border-t border-gray-700 text-center">
            <button onClick={() => setIsRegistering(true)} className="text-blue-400 hover:underline text-sm font-medium">
