...
- **Nueva (jul-2026):** `DOCUMENTACION_COMPLETA.md` (manual técnico V3.0) + la página in-app `/documentacion` coexisten con esta carpeta `/docs` sin referenciarse mutuamente — dos fuentes de documentación técnica paralelas con riesgo de divergencia. Además, el diagrama de arquitectura de ese manual describe la capa de IA como "OpenRouter / Gemini API Edge", reintroduciendo la ambigüedad Gemini/OpenRouter que ya se había aclarado a favor de OpenRouter. Decidir cuál es la fuente autoritativa y enlazar una desde la otra.
